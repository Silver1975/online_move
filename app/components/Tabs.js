...
export default function Tabs({ movie }) {
  ...
  const fetchComments = async (episodeId) => {
    ...
    setError(null);
    try {
      const response = await fetch(`http://localhost:5221/api/Comment?episodeId=${episodeId}`);
      if (!response.ok) {
  ...
  return (
    <div className={styles.tabs}>
      <div className={styles.tabList}>
        ...
        <button
          className={`${styles.tabItem} ${
            activeTab === "related" ? styles.active : ""
          }`}
      ...
      <div className={styles.tabContent}>
        ...
        {activeTab === "episodes" && (
          <div>
            <h3>Episodes</h3>
            {/* Вивести список епізодів */}
            <EpisodesSlider seasonId={movie.id} />
          </div>
        )}
        {activeTab === "cast" && (
          <div>
